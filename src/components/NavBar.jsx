...
const NavBar = () => {
    ...
    return (
     ...
     text-white bg-black fixed'>
            ...
            <div>
                <h1 className='text-5xl font-signature ml-2'>Pranav Arjun</h1>
            </div>
            ...
            <div onClick={() => steNav(!nav)} className='cursor-pointer pr-4 z-10 text-gray-500 md:hidden'>
                {nav ? <FaTimes size={30} /> : <FaBars size={30} />}
            </div>

            {nav && (
                <ul className='flex flex-col justify-center items-center absolute top-0
             left-0 w-full h-screen bg-gradient-to-b from-black to-gray-700'>

                    {links.map(({ id, link }) => (
                        <li key={id} className='px-4 cursor-pointer capitalize py-6 text-4xl
                        text-gray-500 hover:scale-105 duration-200'>
                            <Link onClick={() => steNav(!nav)} to={link} smooth duration={500}>{link}</Link>
                        </li>
                    ))}
